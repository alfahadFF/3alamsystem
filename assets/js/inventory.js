...
/* ================================================================
   ...
   ================================================================ */
function calcAutoConsumption(inv) {
  if (!inv.trackable || !inv.recipe?.length) return 0;
  let total = 0;
  const printedItems = invoices
    .filter(inv => inv.status === 'printed')
    .flatMap(inv => inv.items || []);

  inv.recipe.forEach(r => {
    const sold = printedItems
...
/* حساب الكمية الفعلية = مدخلات - مخرجات يدوية - استهلاك تلقائي */
function effectiveQty(inv) {
  const manualOut = (inv.log || [])
    .filter(l => (l.type === 'out' && !l.auto) || l.type === 'waste')
    .reduce((s, l) => s + (l.qty || 0), 0);
  const autoConsume = calcAutoConsumption(inv);
  return inv.qty - manualOut - autoConsume;
}
  ...
  return `
    <div class="inv-info-grid">
      ...
      <div class="inv-info-cell red">
        ...
        <div class="inv-info-lbl">استهلاك المبيعات</div>
      </div>
      <div class="inv-info-cell">
...
function renderInvRecipe(inv) {
  return `
    ...
    <div class="inv-recipe-note">
      الاستهلاك اليوم من المبيعات: <strong>${fmtQty(calcAutoConsumption(inv))} ${e(inv.unit)}</strong>
    </div>`;
}
...
function saveAddStock(id) {
  ...
  inventory[idx].log.push({
    id: 'il_' + Date.now(), date: new Date().toISOString().split('T')[0],
