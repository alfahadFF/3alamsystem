...
function e(v){ return String(v ?? '').replace(/[&<>'"]/g, ch =>
  ({'&':'&amp;','<':'&lt;','>':'&gt;',"'":'&#39;','"':'&quot;'}[ch])); }
function fmtNum(n){ return Number(n || 0).toLocaleString('en-US'); }
function showToast(msg, icon='✅'){
  const t = document.getElementById('toast'); if (!t) return;
  t.innerHTML = `<span>${icon}</span><span>${e(msg)}</span>`; t.classList.add('show');
...
function todayStr(){ return (window.businessDay ? businessDay() : new Date().toISOString().slice(0,10)); }
function weekStart(){
