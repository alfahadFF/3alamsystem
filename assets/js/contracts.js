...
function renderContent() {
  ...
  const overdue  = contracts.flatMap(c => c.installments || [])
    .filter(i => !i.paid && i.due_date < new Date().toISOString().split('T')[0]).length;
        ...
        <table class="stmt-table">
          ...
          <tbody>
            ${(c.installments||[]).map(i => `
              <tr class="${!i.paid && i.due_date < new Date().toISOString().slice(0,10) ? 'stmt-overdue' : ''}">
                <td>${e(i.due_date)}</td><td>${fmtNum(i.amount)}</td>
                <td>${i.paid ? '✅ مدفوعة' : (i.due_date < new Date().toISOString().slice(0,10) ? '🔴 متأخرة' : '⏳ مستحقة')}</td>
                <td>${e(i.paid_date || '—')}</td>
              </tr>`).join('')}
          </tbody>
        </table>
...
function renderContractRow(c) {
  ...
  const isOverdue = nextDue && nextDue.due_date < new Date().toISOString().split('T')[0];
  ...
  return `
    ...
    <div class="con-inst-list">
      ${installments.map((inst, i) => {
        const isOverdue = !inst.paid && inst.due_date < new Date().toISOString().split('T')[0];
        return `
